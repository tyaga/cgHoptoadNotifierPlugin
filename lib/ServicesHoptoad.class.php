...
class ServicesHoptoad
{
     ...
     */
    public static function installHandlers($apiKey = NULL, $environment = NULL, $client = NULL, $class = 'ServicesHoptoad')
    {
    ...
    {
        $this->notify(get_class($exception), 'Uncaught Exception : '.$exception->getMessage(), $exception->getFile(), $exception->getLine(), $exception->getTrace());
    }
    ...
    function notify($error_class, $message, $file, $line, $trace, $component = NULL)
    {
        ...
        $headers = array(
            'Accept'=>'text/xml, application/xml' , 'Content-Type'=>'text/xml'
        );
        $body = $this->buildXmlNotice();
        
        try
        {
            $status = call_user_func_array(array(
                $this , $this->client.'Request'
            ), array(
                $url , $headers , $body
            ));
    ...
    function buildXmlNotice()
    {
        ...
        @$request->addChild('url', str_replace('&', '', $this->request_uri()));
    ...
    function addXmlVars($parent, $key, $source)
    {
        ...
        {
            @$var_node = $node->addChild('var', str_replace('&', '', (string) $val));
    ...
    function addXmlBacktrace($parent)
    {
        ...
        {
            if (isset($entry['class']) && $entry['class'] == 'ServicesHoptoad') continue;
